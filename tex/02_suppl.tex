...
\begin{equation}
    r_{ik} =
    \begin{cases}
    \min(r_{0} + b u_i, 1) & \text{if $k \in \text{producer}~(k = 1, 2, \ldots, P)$,}\\
    |A_k|^{-1} \sum_{q \in A_k} \pi_{iq} & \text{if $k \in \text{consumer}~(k = P + 1, P + 2, \ldots, S)$,}
    \end{cases}
    \label{eq:r-eq}
\end{equation}

where $r_0$ ($0 \le r_0 \le 1$) is the (minimum) resource supply, and the parameter $b$ ($\ge 0$) expresses the downstream increase of resource supply as a coefficient of the upstream river length at patch $i$ ($u_i$).
...
$A_k$ is the set of prey available for species $k$ (and thus $\sum_{q  \in A_k} \pi_{iq}$ represents the expected number of prey species at patch $i$), and $|A_k|$ is the number of prey species consumable for species $k$.
...
\begin{equation}
    r_{k} = 
    \begin{cases}
        \min(r_0 + b \hat{u}, 1) & \text{if $k \in \text{producer}~(k = 1, 2, \ldots, P)$,}\\
...
In Equations \ref{eq:clnz} and \ref{eq:extn}, we introduced the expected value of upstream river length $\hat{u}$, which can be expressed as a function of total river length $L$ and branching rate $\lambda_b$ (see Section \ref{updist} for derivation).
