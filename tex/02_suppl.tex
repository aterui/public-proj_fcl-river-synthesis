...
With the known sum $\sum_s l_s = L$, the fraction of single link length $\psi_s$ (= $l_s L^{-1}$) follows a Beta distribution as $\psi_s ~|~ z \sim \mbox{Beta}(1, z)$ (\textbf{Lemma 1}), whose expected value is $\mathbb{E}(\psi~|~z) = (z + 1)^{-1}$.
...
Then, the conditional distribution of $\psi = X_1(X_1 + X_2)^{-1}$ given $X_1 + X_2 = y$ follows $\mbox{Beta}(\alpha, \beta)$.
