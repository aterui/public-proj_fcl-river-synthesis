...
\begin{equation}
    ...
    \frac{\sum_{q \in A_k} \pi_{iq}}{|A_k|} & \text{if $k \in \text{consumer}$,}
    \end{cases}
    \label{eq:r-eq}
\end{equation}

where $r_0$ ($0 \le r_0 \le 1$) is the resource supply, $A_k$ is the set of prey species available for species $k$ (and thus $\sum_{q  \in A_k} \pi_{iq}$ represents the expected number of prey species at patch $q$), and $|A_k|$ is the number of prey species consumable for species $k$.
...
\begin{equation}
    \mu_{ik} = 
        ...
        \underbrace{\mu_{k}^{(p)} \left(1 - \frac{\sum_{q \in A_k} \pi_{iq}}{|A_k|} \right)}_{\text{Prey scarcity}} + 
        \underbrace{\mu_{k}^{(c)} \sum_{q \in \tilde{A}_k} \pi_{iq}}_{\text{Predation}},
    \label{eq:extn}
\end{equation}

where $\rho$ and $u_i$ denote the disturbance synchrony probability and the upstream river length at patch $i$, respectively, and $A_k$ ($\tilde{A}_k$) is the set of consumable prey (predator) species for species $k$.
$|A_k|$ denotes the number of consumable prey species.
...
The extinction risk caused by the lack of prey is reduced by the factor $|A_k|^{-1} \sum_{q \in \text{prey}} \pi_{iq}$ ($\le 1$), which is the ratio of the expected species richness of available prey relative to the number of consumable prey species $|A_k|$.
...
The parameter $\mu_{k}^{(c)}$ defines how quickly the extinction rate increases with the expected species richness of predators $\sum_{q \in \tilde{A}_k} \pi_{iq}$.
...
where $\gamma_k = r_{k} c_{0, k}$ and

\begin{equation}
    r_{k} = 
    \begin{cases}
        r_0 & \text{for producers,}\\
        \frac{\sum_{q \in A_k} p_{q}}{|A_k|} & \text{for consumers,}
    \end{cases}
\end{equation}
...
\begin{lemma}
\label{lemma-f}
Let $l_1, l_2, \ldots, l_b$ be independent random variables that follow $\mbox{Exp}(\lambda_b)$.
If the total length is given as $L=\sum_{s=1}^b l_s$, the fraction of single link length $F_s = l_s L^{-1}$ follows a Beta distribution $\mbox{Beta}(1, z)$.
\end{lemma}

\begin{proof}
An exponential distribution $\mbox{Exp}(\lambda_b)$ is a special case of a Gamma distribution, i.e., $\mbox{Ga}(1, \lambda_b)$.
...
Under the preferential prey model, species $k$ may consume a subset of species $1,2,\ldots, k-1$ but cannot consume $k+1, k+2, \ldots, S$ because the food web is acyclic (see Methods in the main text).
This implies that $\mu_k$ and $\gamma_k$ depend on at least one of $p_1, p_2, \ldots, p_{k-1}$, but they are independent of $p_{k+1}, p_{k+2}, \ldots, p_{S}$.

The Jacobian matrix of Equation \ref{eq:master} is then lower-triangular. The characteristic equation of this Jacobian matrix at equilibrium is given as:
...
Equation \ref{eq:eigenvalue} indicates that $\mu_k - \gamma_k < 0$ for persistent species ($p_k = p_k^*>0$), and $\gamma_k - \mu_k < 0$ for extinct species ($p_k=0$), respectively, at a stable equilibrium.

Now, we show the uniqueness of the stable equilibrium despite the system having multiple equilibria that differ in the combination of the persistent and extinct species.
For producer species ($k=1,2,\ldots, P$), $\mu_k-\gamma_k$ is constant as follows:

\begin{align}
    \mu_k-\gamma_k = r_0 c_{0, k} - \mu_{k}^{(0)} (1 + \rho \hat{u}).
\end{align}
...
where $A_k$ is a set of consumable prey for species $k$.

Recall that $A_k$ does not contain species $q = k, k+1, \ldots, S$ due to the acyclic nature of the food web.
...
This logic recursively holds for species $k=P+2. \ldots, S$.
...
\begin{equation}
    \mu_{k} = 
        ...
        \underbrace{\mu_{k}^{(1)} \left(1 - \frac{\sum_{q \in \text{prey}} p_{q}}{S_{p, k}} \right)}_{\text{Prey scarcity}} + 
