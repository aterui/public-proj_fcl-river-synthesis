...
In the absence of predation effects ($\mu^{(2)} = 0$),-- can be analytically determined by sequentially solving Equation \ref{eq:model0} for $p_k$ from the base to the apex of the food web (Methods). Although our model has multiple equilibria, it has a single stable equilibrium. We evaluated the FCL, defined as the maximum trophic position, at the stable equilibrium; see SI Appendix for the details.
...
\begin{equation}
    ...
    1+\frac{1}{|A_{k}|} \sum_{q \in A_{k}} \tau_q & ~\text{for consumers}~ (k = P+1, P+2, \ldots, S),
    \end{cases}
    % \label{eq:def_tp}
\end{equation}
...
\begin{equation}
    ...
    \begin{cases}
        ...
        \frac{1}{|A_{k}|} \sum_{q~\in A_{k}} p_{q} & ~\text{for consumers}~ (k=P+1, P+2, \ldots, S)
    \end{cases}
\end{equation}
...
In the absence of extinction caused by predation ($\mu_{k}^{(2)}=0$), equilibrium of the system  is analytically derived as follows.
...
For producers ($k=1, 2, \ldots, P$), $\gamma_k$ and $\mu_k$ are constant. $p_k$ at equilibrium is, therefore, binary: present ($p_k=p_k^*>0$) or absent ($p_k=0$).
For consumers ($k=P+1, P+1, \ldots, S$), $p_1, p_2, \ldots, p_{k-1}$, but not depend on $p_{k}, p_{k+1}, \ldots, p_{S}$, can affect $p_1, p_2, \ldots, p_{k-1}$. In other words, when species $k$ is present, the fraction of the patches occupied by species $k$ at equilibrium varies depending those by species $1, 2, \ldots, k-1$. 
Although this implies the system has multiple equilibria, SI Appendix shows that there exist a unique stable equilibrium among them. This leads the unique ``realized'' FCL at equilibrium (see below).
