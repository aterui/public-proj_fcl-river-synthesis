...
The key component is the expected value of upstream river length $\hat{u}$.
Assuming a finite total river length $L > 0$ with random branching at a rate $\lambda_b > 0$, the expected value $\hat{u}$ emerges as a nonlinear function of $L$ and $\lambda_b$ (see Figure \ref{fig:scheme}B for visual representation and \textit{SI Appendix} for derivation):
