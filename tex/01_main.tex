...
Without imposing any additional assumptions, the spatial mean-field approximation naturally yields the effective propagule $\phi_k$ as a function of the expected network distance $\hat{d} \equiv \mathbb{E}\bigl(d_{ij}\bigr)$, which exhibits the asymptotic scaling with $L$ and $\lambda_b$ (Proposition XXX for the mathematical proof):
...
\label{eq:phi-pp-pgle}
\end{equation}

where $\mathrm{erfcx}(\cdot)$ is the scaled complementary error function, and the constant $g_k$ quantifies the propagule production per habitat such that the product $g_k N$ represents the global pool of dispersing propagules in the ecosystem.
The effective propagule $\phi_k$ therefore emerges as an explicit function of ecosystem size and complexity.

Equation \ref{eq:phi-pp-pgle} suggests that total river length $L$ increases the global pool of propagules (recall $N = Lh$); however, the fraction of effective propagules declines with increasing $L$, because many habitats become too distant to function as sources, particularly when dispersal ability is limited (i.e., larger $\delta_k$).
...
The parameters $\mu^{(0)}$ and $\mu^{(1)}$ determine the baseline contributions of disturbance and prey scarcity to local extinction, respectively (see \textit{SI Appendix} for the full derivation).
...
The metapopulation model in Equation \ref{eq:pp-model} is therefore derived rigorously from the mechanistic assumptions at the local habitat level, rather than being specified heuristically at the metapopulation level.
...
More importantly, our analysis provided insights into the roles of ecosystem size and complexity in regulating this predator-prey system.
The influence of ecosystem size is highly context dependent, whereas ecosystem complexity consistently has a positive effect on the occupancies of both species.
...
By contrast, increasing branching consistently promotes the occupancies by providing a dual advantage: it shortens dispersal pathways, thereby enhancing colonization, while simultaneously compartmentalizing disturbance propagation and reducing synchronized extirpation.
...
% The metapopulation model in Equation \ref{eq:pp-model} is obtained as a spatial mean-field approximation of the local occupancy dynamics at individual habitats \citep{ovaskainen_asymptotically_2006}, and this standard derivation step establishes a connection with ecosystem properties.
...
However, the predicted relationship between FCL and total river length ($L$) is either hump-shaped or vague, illuminating the context dependency of ecosystem size influences (Figures \ref{fig:sim-main}A and \ref{fig:sim-main}C).
