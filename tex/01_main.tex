...
Here, we use theory and analysis of cross-continental FCL data to demonstrate that ecosystem complexity, rather than ecosystem size, plays a broader role in regulating food chains in rivers.
...
For example, dendritic ecosystems such as trees and rivers show deep self-similarity in branching patterns -- a hallmark of fractals \citep{rodriguez-iturbe_fractal_2001, terui_revisiting_2024} -- meaning that this geometric feature is effectively decoupled from ecosystem size.
...
These influences of ecosystem complexity should propagate through predator-prey interactions, ultimately shaping the vertical organization of food webs.
...
Our theoretical predictions are supported by the results of an analysis of cross-continental FCL data, which provides strong empirical evidence for our hypothesis that ecosystem complexity regulates FCL.
...
Under the assumption of a finite-sized network with random branching, we derived that the expected value of upstream river length $\hat{u}$ is a nonlinear function of total river length $L$ and branching rate $\lambda_b$ (Figure \ref{fig:scheme}B; Methods).
...
Our synthesis distinguishes itself from similar previous studies in its extent, extracting FCL estimates at 317 sites across 115 watersheds worldwide (Figure \ref{fig:fcl-obs}A).
...
Our advanced statistical modeling accounted for common challenges in analyzing data from the literature, such as outliers, unequal and non-random sampling efforts, and the imperfect sampling of stable isotope data (see Methods for details).
...
We chose this method because it generates (i) acyclic food webs, (ii) food web structures comparable to those observed in nature, and (iii) plausible ranges of FCL values with parameter values informed by empirical food webs \citep{shibasaki_food_2024}.
...
The parameter $\rho$ controls the likelihood of upstream-downstream co-extirpation and is independent of ecosystem size and complexity.

The key component of our model is the expected value of upstream river length $\hat{u}$.
Assuming a finite total river length $L > 0$ with random branching at a rate $\lambda_b > 0$, the expected value $\hat{u}$ becomes a nonlinear function of both $L$ and $\lambda_b$ (see \textit{SI Appendix} for derivation):
...
where $z$ is the number of links minus one and $2\mathbb{Z}_{\ge 0}$ denotes the set of positive even integers ($z \in 2\mathbb{Z}_{\ge 0}$ is a constraint arising from the requirement that the number of links be odd).
...
It is important to note that Equations \ref{eq:u-hat}–\ref{eq:t-poisson} represent an analytical solution that holds for any finite-sized network with random bifurcation.
