...
This framework uses probabilistic theory to account for stochastic variation in network topology and link lengths under a given level of ecosystem size and complexity.
...
\begin{figure}
    ...
    \caption{Modeling scheme. (A) Schematic representation of modeled branching networks. Each link represents a river segment, with confluences indicating where segments meet. Habitat patches (black dots) are randomly distributed along the network. The upstream river length $u$ (black solid lines) refers to the portion of the river located upstream from a given habitat patch (the expected value is on the Y-axis in panel B). As ecosystem size increases, total river length ($L$) expands without affecting the branching rate ($\lambda_b$). In contrast, increasing the branching rate raises ecosystem complexity while keeping the total river length constant. (B) The upstream river length $u$ has the expected value $\hat{u}(L, \lambda_b)$, which is a nonlinear function of total river length $L$ and branching rate $\lambda_b$ (see Equation \ref{eq:u-hat} in Methods and \textit{SI Appendix} for derivation).}
    \label{fig:scheme}
\end{figure}
...
In contrast, increased branching with higher $\lambda_b$ compresses the river network topology, shortening dispersal pathways and enhancing the effective number of propagule sources.
...
According to Equation \ref{eq:mu-pp}, the disturbance rate increases with increasing $L$. 
...
Meanwhile, branching divides the river network into flow-unconnected sub-networks, thereby dispersing the risk of synchronized extirpation due to flow-mediated disturbances.
...
This simplified formulation highlights the unique contributions of our modeling framework by allowing the effects of river-network structure to be examined without confounding differences in species-specific traits.
