...
\subsection{Predator-Prey Metapopulation Model}
...
The purpose of this analysis is to understand the model's behavior under a simplified scenario. 
...
Our analysis provided insights into the roles of ecosystem size and complexity in regulating the predator-prey system.
Increasing branching consistently promoted the occupancies by providing a dual advantage: it shortens dispersal pathways, thereby enhancing colonization, while simultaneously compartmentalizing disturbance propagation and reducing synchronized extirpation.

The influence of ecosystem size was highly context dependent, largely modulated by species dispersal capacity.
When dispersal range is limited ($\delta = 0.1$), increasing total river length has a neutral to slightly negative effect on the occupancies because the colonization benefits of larger ecosystems are largely offset by the increased risk of downstream disturbance cascades. 
Increasing dispersal distance ($\delta = 0.05$) shifts the effect of ecosystem size to positive by facilitating colonization from distant source habitats.
However, this positive effect is confined to very small ecosystems, with occupancy rapidly saturating as ecosystem size increases.
These patterns remained consistent even when varying the basal disturbance rate $\mu^{(0)}$ and the spatial scale of disturbance cascade $\nu$.
...
Consequently, generalist consumers have more suitable habitats than specialist consumers.

While $\phi_k$ remains as defined in Equation~\ref{eq:phi-pgle}, the establishment probability $c_k$ and the extinction rate $\mu_k$ were extended to capture the trophic complexity of food webs.
