...
The complexity of tributary branching consistently had positive effects on riverine food chains worldwide, whereas the total river length showed no clear relationships.
...
Our model indicates, however, that the predicted relationship between FCL and total river length ($L$) is either hump-shaped or vague, which differs substantially from the prediction of the ecosystem size hypothesis (Figure \ref{fig:sim-main}A and C).
...
Ecosystem size and complexity were measured at the watershed level, where the total river length and the branching rate represent the properties of an entire watershed within which sampling sites are located (Methods).
...
Such anomalies, whether in the form of flooding or drying, can increase the likelihood of species extirpation.
Further, a recent study has shown that high flow variability can reduce energy transfer efficiency within a food web, leading to shorter FCL \citep{baruch_food_2023}.
These processes may act in concert to constrain FCL in rivers.
...
In contrast, the total river length quantifies the size influences at a broader landscape scale (i.e., watershed level), where statistical averaging may attenuate local variations. 
