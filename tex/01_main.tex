...
\section{Maintext}
Since Charles Elton introduced the term ``food cycles'' in 1927, food webs have been a central theme in ecology \citep{paine_food_1966, pimm_food_1991, post_long_2002}.
...
Current debates on FCL, however, predominantly assumed simplified landscapes for tractability, overlooking the role of geometric ecosystem complexity (``ecosystem complexity'').
...
The analysis provided strong statistical support for the effect of branching rate, with a high probability ($\approx$ 0.95) that the regression coefficient is positive (Figure \ref{fig:fcl-obs}A).
Conversely, the effect of total river length was ambiguous (Figure \ref{fig:fcl-obs}B), characterized by the posterior distribution of its coefficient centered around zero.
Interestingly, despite continental uniqueness in ecological communities and distinct evolutionary histories, no statistical evidence exists that these relationships vary among them -- the model assuming constant slopes for these ecosystem properties exhibited the WAIC value lower than one with continent-specific slopes (-251.4 vs. -251.2).
