...
\begin{equation}
    \mu_{k} = 
        ...
        \underbrace{\mu_{k}^{(p)} \left(1 - \frac{1}{|S_{k}|}\sum_{q\in S_k} p_{q} \right)}_{\text{Prey scarcity}} + 
        \underbrace{\mu_{k}^{(c)} \sum_{q~\in~\text{predator of }k} p_{q}}_{\text{Predation}},
    \label{eq:extn}    
\end{equation}

where $\mu_k^{(0)}$, $\mu_k^{(p)}$, and $\mu_k^{(c)}$ are base parameters scaling the impacts of disturbance, prey scarcity, and predation, $\rho$ is the spatial synchrony probability, $\hat{u}$ is the expected upstream river length at a given habitat patch within a branching network, $S_{k}$ is the set of consumable prey for consumer $k$, and $|S_{k}|$ is the number of such prey species for consumer $k$.
For simplicity, we assumed constant values across species for $\mu_k^{(0)} = \mu^{(0)}$, and $\mu_k^{(c)} = \mu^{(c)}$. $\mu_k^{(p)}$ was assumed zero for all producers and constant ($\mu_k^{(p)} = \mu^{(p)}$) for all consumers.

The first term of Equation 3 %\ref{eq:extn}
...
In the absence of predation effects ($\mu^{(c)} = 0$), the FCL at equilibrium -- defined as the maximum trophic position (see Equation \ref{eq:def_tp} for its definition) of persistent species -- can be analytically determined by sequentially solving Equation \ref{eq:model0} for $p_k$ ($dp_k/dt = 0$) from the base to the apex of the food web (Methods). 
...
Although the niche model \citep{williams_simple_2000} is used widely in food web research, this algorithm is unsuitable for our study because it may create loops and cannibalism in a food web, which prevented us from defining trophic positions and FCL \citep{shibasaki_food_2024}.
...
Producer trophic positions are ones, and
...
\begin{equation}
    ...
    1+\frac{\sum_{q~\in~ S_k} \tau_q}{|S_{k}|} & ~\text{for consumers}~ (k=P+1, P+2, \ldots, S),
    \end{cases}
    \label{eq:def_tp}
\end{equation}

where $S_{k}$ is the the of consumable prey species for consumer $k$, and $|S_k|$ is the number of such prey species for  consumer $k$.
...
\begin{equation}
    ...
    \begin{cases}
        ...
        \frac{\sum_{q~S_k} p_{q}}{| S_{k}|} & ~\text{for consumers}~ (k=P+1, P+2, \ldots, S)
    \end{cases}
\end{equation}

where $r_0$ ($0 \le r_0 \le 1$) is the resource supply, and $\sum_{q~\n ~ S_k p_{q}}$ ($q \ne k$) is the expected species richness of prey for consumer $k$.
...
The expected value $\hat{u}$ can be expressed as a nonlinear function of $L$ and $\lambda_b$ (see SI Appendix for derivation):
...
In the second term of Equation \ref{eq:extn}, we assume that the extinction rate decreases with increasing species richness of available prey, and the parameter $\mu_{k}^{(p)}$ denotes the extinction rate in the complete absence of prey ($\mu_{k}^{(p)} = 0$ for producers).
Predation pressure is assumed to increase linearly with the species richness of predators $\sum_{q~\in~\text{predator of } k} p_{q}$.
...
\begin{equation}
    \tau^{(w)}_k = \frac{\sum_{q~\in~S_k} p_{q} \tau^{(w)}_q}{\sum_{q~\in~S_k} p_{q}} + 1
\end{equation}
...
While Equation \ref{eq:watershed-avarage} represents a random intercept model, we also developed a random intercept-slope model as an alternative model, in which the effects of ecosystem size and complexity vary by region (SI Appendix).
