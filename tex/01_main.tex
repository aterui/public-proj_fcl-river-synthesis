...
In our framework, river networks are characterized by total river length $L$ (ecosystem size) and branching rate $\lambda_b$ (ecosystem complexity).
...
This framework uses probabilistic theory to account for stochastic variation in network topology and link lengths under a given level of ecosystem size and complexity (Figure \ref{fig:scheme}A).
The results presented below should therefore be interpreted as general predictions emerging from the full ensemble of possible river network configurations, rather than a limited subset of realized networks.
...
\begin{figure}
    ...
    \caption{Modeling scheme. (A) Schematic representation of the modeled branching networks. Ecosystem size is increased by expanding total river length $L$ without changing branching rate $\lambda_b$. Conversely, ecosystem complexity is increased by increasing branching rate while maintaining constant total river length. A ``link'' represents a river segment, and confluences indicate where segments meet. Habitat patches (black and open dots) are randomly distributed along the network. The upstream river length $u$ (black solid lines) is the portion of the river located upstream of a focal habitat patch (black dot). The network distance $d$ is the distance measured along the river network between two habitat patches (black and open dots). (B) Effective propagule $\phi_k$ (Equation \ref{eq:phi-pgle}) increases with both total river length $L$ and branching rate $\lambda_b$. In contrast, the disturbance rate ($\mu^{(0)}(1 + \hat{\rho}\hat{u})$; see Equation \ref{eq:mu-pp}) increases with total river length but decreases with branching rate. For illustration, we set $h = 1$, $g = 1$, $\delta = 0.1$, and $\nu = 0.1$, and $\mu^{(0)} = 1$.}
    \label{fig:scheme}
\end{figure}

\noindent\makebox[\linewidth]{\rule{\textwidth}{0.4pt}}

Here, we use a metapopulation model for a predator–prey system (a single predator and its prey) to illustrate how colonization-extinction dynamics are regulated by ecosystem size and complexity.
...
where the constant $c_k$ is the per-propagule establishment probability, $\phi_{k}$ is the number of effective propagules (i.e., surviving propagules after dispersal), and $\mu_{k}$ is the extinction rate ($k \in \{1,2\}$).
...
Without imposing any additional assumptions, the spatial mean-field approximation yields the effective propagule $\phi_k$ as a function of the expected network distance $\hat{d} \equiv \mathbb{E}\bigl(d_{ij}\bigr)$, which exhibits the asymptotic scaling with $L$ and $\lambda_b$ (Proposition XXX for proof):
...
% The effective propagule $\phi_k$ naturally emerges as an explicit function of ecosystem size and complexity.
...
Here, $\hat{u}$ asymptotically scales as $\hat{u} \propto L^{\frac{1}{2}}\,\lambda_b^{-\frac{1}{2}}$ (Proposition XXX in \textit{SI Appendix} for proof), and $\hat{\rho}$ is a non-linear function of $L$ and $\lambda_b$.
...
These patterns remained consistent across varying basal disturbance rates ($\mu^{(0)}$) and spatial scales of disturbance cascades ($\nu$). 
...
Our approach required several simplifying assumptions for the sake of analytical tractability.
...
where $\eta_k$ is the fraction of ``suitable'' habitats for species $k$.
For producers, $\eta_k = 1$ as all habitats are assumed to be habitable.
For consumers, $\eta_k$ represents the fraction of habitats occupied by at least one prey species, $\eta_k = 1 - \prod_{q \in A_k}(1-p_q)$, where $A_k$ denotes the set of consumable prey for consumer $k$ and $q$ is an index denoting each prey species in $A_k$.
While $\phi_k$ remained as defined in Equation~\ref{eq:phi-pgle}, we allowed propagule production and dispersal capability to vary with species trophic position $\tau_k$ according to $g_k \propto \tau_k^{-\psi}$ and $\delta_k \propto \tau_k^{-\psi}$, respectively. 
...
We assumed that establishment depends on the downstream accumulation of resources for producers and on the local richness of consumable prey for consumers.
...
where $r_0$ is the resource supply, $b$ is the downstream accumulation rate of resources, $\hat{s}_k$ is the expected richness of consumable prey for consumer $k$, and $|A_k|$ is the number of consumable prey (or ``diet breadth'').
...
The division by the $k$'s diet breath $|A_k|$ expresses that consumer's establishment is proportional to relative prey availability.

The bottom-up influence of prey on extinction was assumed to depend on the relative scarcity of prey at individual habitats, yielding the following expression at the metapopulation level:

\begin{equation}
    \mu_{k} = 
        \underbrace{\mu^{(0)} (1 + \rho \hat{u})}_{\text{Disturbance}} + 
        \underbrace{\mu_k^{(1)} \left(1 - \frac{\hat{s}_k}{|A_{k}|}\right)}_{\text{Prey scarcity}}.
    \label{eq:extn-main}    
\end{equation}

In this formulation, the extinction rate increases as $\hat{s}_k$ decreases, while the effect of losing a single prey species is smaller for consumers with broader diets (i.e., larger $|A_k|$).
Thus, despite its simplicity, this formulation captures species-specific vulnerability to prey limitation.
For simplicity, the prey effect $\mu_k^{(1)}$ was assumed to be identical for all consumers ($\mu_k^{(1)} = \mu^{(1)}$) and absent for all producers ($\mu_k^{(1)} = 0$).

We analyzed the generalized model (Equation \ref{eq:model-fw}) using 20 replicas of simulated food webs.
We used the prey-preference model, which generates acyclic food webs with omnivory.
It allows the set of consumable prey species to vary among consumers, capturing the trophic complexity characteristic of natural food webs.
...
The unique FCL at stable equilibrium can be determined analytically by recursively solving Equation \ref{eq:model-fw} for $p_k$ from the base to the apex of the food web (Methods and \textit{SI Appendix}).

Our model analysis predicts that the effects of ecosystem structure are transmitted through predator–prey interactions, extending the mechanisms identified in the simple predator–prey system to food chains. 
The dual effect of branching, i.e., enhanced connectivity and reduced spatial coherence of disturbances, consistently increased FCL across ecological scenarios. 
In contrast, the effect of ecosystem size (total river length $L$) was largely neutral (Figure \ref{fig:sim-main}A and B), with positive effects emerging only within a limited range where increased propagule supply outweighs the elevated risk of disturbance cascades.
...
The analytical predictions require the assumption of no predation effects on the occupancy dynamics.
However, numerical simulations suggest that our analytical results are robust even when including predation on the extinction rate and allowing variation in other key ecological parameters, including dispersal scale ($\delta_k$), prey effect ($\mu^{(1)}$), the spatial scale of disturbance cascades ($\nu$), and trophic omnivory ($\theta$).
The positive relationship between FCL and ecosystem complexity (branching rate) was consistent across all cases, whereas the influence of ecosystem size on FCL remained negligible to weakly positive (Figures S2–S9, \textit{SI Appendix}).
...
Disturbance cascades play a central role in generating the weak effect of ecosystem size. When their influence is removed from the model by treating the disturbance term in Equation \ref{eq:extn-main} as constant (see \textit{SI Appendix} for details), the predicted relationship reduces to classical expectations: ecosystem size exerts a strong positive effect on FCL (Figures \ref{fig:sim-main}C and S10). This result is consistent with established food chain theory \citep{holt_food_2002, takimoto_effects_2012, terui_spatial_2019} and empirical observations in systems where spatially cascading disturbances are weak, such as lake ecosystems \citep{post_ecosystem_2000, takimoto_ecosystem_2008, doi_resource_2009}.
...
While watershed area can be an alternative proxy for ecosystem size \citep[e.g.,][]{sabo_role_2010}, total river length is strongly and linearly correlated with watershed area (Pearson's r = 0.99). 
...
We also accounted for potential influences of resource supply, disturbance, and human impacts through relevant proxy variables (elevation, precipitation, and human footprint index, respectively; Methods).
...
Under these regimes, tributaries serve as refugia from spatial disturbance cascades \citep[e.g.,][]{koizumi_fishermen_2013}, with confluences supporting longer food chains \citep{sabo_pulsed_2018, sullivan_ecosystem_2015}. 
...
The analysis was performed in R 4.5.2 \citep{r_program}.
