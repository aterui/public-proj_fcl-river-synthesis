...
list_table <- lapply(list_est, function(data) {
  ...
  data %>% 
    ...
    mutate(symbol = case_when(varname == "local_elev" ~ "$\\alpha_1$",
                              ...
                              parms == "sigma_b[1]" ~ "$\\sigma_{\\beta0}$",
                              parms == "sigma_b[2]" ~ "$\\sigma_{\\beta1}$",
                              parms == "sigma_b[3]" ~ "$\\sigma_{\\beta2}$",
                              parms == "z[1]" ~ "$\\xi_{1}$",
                              parms == "z[2]" ~ "$\\xi_{2}$"),
           ...
           xtable(list_table[[i]],
                  caption = 
                    paste0(
                      ...
                      " (", m[i], ") ",
                      "with corresponding posterior standard deviations (SDs) ",
                      #"with corresponding 95\\% credible intervals (CIs)",
                      "and probabilities ($\\Pr(\\cdot)$), representing the uncertainty around each parameter estimate." 
                    ),
