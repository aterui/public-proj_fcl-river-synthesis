...
df_parms <- lapply(list_parms, FUN = function(x) {
  ...
  mutate(symbol = case_when(parms == "rsrc" ~ "$r_0$",
         ...
         description = case_when(parms == "rsrc" ~ "Resource supply [-]",
                                 parms == "zeta" ~ "Effect of stream size on the establishment of basal species [per unit river distance]",
                                 parms == "g" ~ "Number of propagules for producers [-]",
                                 parms == "h" ~ "Habitat density [per unit length]",
                                 parms == "delta0" ~ "Dispersal capability for producers [unit length]",
